...
filtered_df = df.filter(df.produit == product_name).repartition(4).cache()  # Répartition sur 4 partitions
...
for i in range(nb_executions):
    ...
    results = filtered_df.select("nom_entreprise", "produit", "ville", "localisation_site.coordinates").collect()  # Sans limit
    distances = []
    for row in results:
