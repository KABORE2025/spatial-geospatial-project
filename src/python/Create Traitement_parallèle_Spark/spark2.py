...
spark = SparkSession.builder \
    ...
    .master("local[*]") \
    .config("spark.mongodb.input.uri", "mongodb://localhost:27017/BigData.cluster") \
    .config("spark.jars.packages", "org.mongodb.spark:mongo-spark-connector_2.12:3.0.1") \
    .getOrCreate()

# Paramètres
product_name = "Tracteur"
...
filtered_df = df.filter(df.type_marque == product_name).cache()
...
for i in range(nb_executions):
    ...
    results = filtered_df.select("nom_entreprise", "type_marque", "ville", "localisation_site.coordinates").limit(5).collect()
    distances = []
    for row in results:
    ...
    print(f"\nExécution {i+1} :")
    print(f"{'Pos':<5} {'Nom':<20} {'type_marque':<20} {'Ville':<15} {'Coordonnées':<35} {'Distance':<10}")
    print("-" * 105)
    for j, (row, distance) in enumerate(distances, 1):
        distance_str = f"{distance:.2f} km" if distance is not None else "Invalide"
        print(f"{j:<5} {row['nom_entreprise']:<20} {row['type_marque']:<20} {row['ville']:<15} {str(row['coordinates']):<35} {distance_str:<10}")
